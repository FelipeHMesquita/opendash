...
import type { SignUpConfig, ItemConfig, Page } from "@/app/_builder-state"

const CloseIcon = () => (
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M18 6 6 18M6 6l12 12" /></svg>
)

export function SignUpConfigPopover({
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between">
                <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">Configurar Sign Up</span>
            ...
            <div className="space-y-2">
                ...
                <label className="text-[10px] font-medium text-muted-foreground mb-1 block">Label Nome</label>
                <input value={config.nameLabel} onChange={e => onChange({ nameLabel: e.target.value })}
                    className="w-full h-7 rounded border border-border bg-background px-2 text-xs text-foreground outline-none focus:ring-1 focus:ring-primary" />
                ...
                <label className="text-[10px] font-medium text-muted-foreground mb-1 block">Texto termos</label>
                <input value={config.termsText} onChange={e => onChange({ termsText: e.target.value })}
                    className="w-full h-7 rounded border border-border bg-background px-2 text-xs text-foreground outline-none focus:ring-1 focus:ring-primary" />

                <label className="text-[10px] font-medium text-muted-foreground mb-1 block">Link termos</label>
                <input value={config.termsLink} onChange={e => onChange({ termsLink: e.target.value })}
                    className="w-full h-7 rounded border border-border bg-background px-2 text-xs text-foreground outline-none focus:ring-1 focus:ring-primary" />

                <label className="text-[10px] font-medium text-muted-foreground mb-1 block">Link privacidade</label>
                <input value={config.privacyLink} onChange={e => onChange({ privacyLink: e.target.value })}
                    className="w-full h-7 rounded border border-border bg-background px-2 text-xs text-foreground outline-none focus:ring-1 focus:ring-primary" />
            ...
            <div className="space-y-2 border-t border-border pt-3">
                ...
                <label className="text-[10px] font-medium text-muted-foreground mb-1 block">Entrar → Página</label>
