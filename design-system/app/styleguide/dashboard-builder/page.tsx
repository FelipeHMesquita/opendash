...
import { themes, type ThemeName, ALL_THEMES, THEME_SWATCHES } from "../_themes"
...
function generateExportText(
    pages: Page[], navLinks: NavLink[], themeName: string,
    globalVariantLabel: string, gridCols: number,
    showNavbar: boolean, showSidebar: boolean, mockSidebarWidth: number,
): string {
    ...
    const lines = ["# Referência de Layout — Protótipo", ""]

    // Theme
    lines.push("## Tema Aplicado", `- Nome: ${themeName}`)
    if (themeVars) Object.entries(themeVars).forEach(([k, v]) => lines.push(`- ${k}: ${v}`))
    lines.push("")

    // Layout
    lines.push("## Layout")
    lines.push(`- Navbar: ${showNavbar ? "visível" : "oculta"}`)
    lines.push(`- Sidebar: ${showSidebar ? `visível (${mockSidebarWidth}px)` : "oculta"}`)
    lines.push(`- Grid: ${gridCols} colunas`)
    lines.push("")
    ...
    if (pages.length > 1) {
        ...
        function printTree(nodes: ReturnType<typeof buildTree>, indent = "") {
            for (const node of nodes) {
                const slug = node.label.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "")
                lines.push(`${indent}- /${slug} — ${node.label}`)
    ...
    pages.forEach(page => {
        ...
        page.canvasItems.forEach((item, i) => {
            ...
            lines.push(
                `#### ${i + 1}. ${item.name}${vSuffix}`,
                `- Import: \`${importStmt}\``,
                `- Largura: ${item.colSpan}/${gridCols} colunas`,
                ...(item.heightPx > 0 ? [`- Altura: ${item.heightPx}px`] : []),
                `- Dados: \`${item.dataType}\``,
                "",
            )
        })
    ...
    if (navLinks.length > 0) {
        ...
        for (const link of navLinks) {
            ...
            const source = link.sourceType === "mock-navbar" ? "Navbar" : "Sidebar"
            const slug = targetPage.label.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "")
            lines.push(`- ${source} "${link.sourceItemLabel}" → /${slug} (${targetPage.label})`)
        }
        lines.push("")
    }
    ...
    lines.push(
        "## Instruções para IA",
        "Implemente em Next.js App Router. Cada página é uma route.",
        pages.length > 1 ? "A hierarquia do sitemap define o nesting de routes." : "",
        navLinks.length > 0 ? "Use o mapa de navegação para Links reais." : "",
        "Substitua DATA pelos dados reais. Aplique as CSS variables do tema em :root.",
    )
...
export default function DashboardBuilderPage() {
    // ── Builder state (pages + canvas items + nav links) ─────────────────────
    const [state, dispatch] = React.useReducer(builderReducer, undefined, createInitialState)
    ...
    const [globalVariantLabel, setGlobalVariantLabel] = React.useState("Default")
    ...
    const [mockSidebarOpen,   setMockSidebarOpen]   = React.useState(true)
    const [mockSidebarWidth,  setMockSidebarWidth]  = React.useState(192)

    // Grid
    const [showGrid,    setShowGrid]    = React.useState(false)
    const [gridCols,    setGridCols]    = React.useState(12)
    const [gridGap,     setGridGap]     = React.useState(32)
    const [gridOpacity, setGridOpacity] = React.useState(20)

    // Content margins
    const [padV, setPadV] = React.useState(24) // top + bottom
    const [padH, setPadH] = React.useState(24) // left + right
    ...
    function handleExport() {
        const text = generateExportText(state.pages, state.navLinks, activeTheme, globalVariantLabel, gridCols, showNavbar, showSidebar, mockSidebarWidth)
        ...
        <div className="flex h-screen flex-col overflow-hidden bg-background text-foreground">
            ...
            <header className="shrink-0 flex items-center gap-4 border-b border-border bg-card px-6 h-12">
                <div className="flex items-center gap-3 shrink-0">
                    <Link href="/styleguide" className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors"><BackIcon /> Voltar</Link>
                    <div className="h-4 w-px bg-border" />
                    <span className="text-sm font-semibold">Dashboard Builder</span>
            ...
            <div className="shrink-0 flex items-center border-b border-border bg-card/60 px-4 h-9 text-xs gap-1 overflow-x-auto">
                ...
                <div className="flex items-center gap-1 px-2 shrink-0">
                    {DEVICE_PRESETS.map(d => {
                        const Icon = d.id === "desktop" ? DesktopIcon : d.id === "tablet" ? TabletIcon : MobileIcon
                        return (
                            <button key={d.id} onClick={() => setDeviceId(d.id)}
                                title={d.width > 0 ? `${d.label} — ${d.width}px` : d.label}
                                className={cn("flex items-center gap-1.5 px-2 py-1 rounded transition-colors", deviceId === d.id ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground")}>
                                <Icon /> {d.label}
                                {deviceId === d.id && d.width > 0 && <span className="text-[10px] text-primary/70">{d.width}</span>}
                            </button>
                        )
                    })}
                </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                <DndContext sensors={sensors} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
                    ...
                    <div className="flex-1 overflow-auto bg-muted/30 p-8">
                        <div
                            className={cn("mx-auto bg-background overflow-hidden flex flex-col ring-2 ring-border shadow-[0_0_0_1px_var(--border),0_0_40px_-8px_var(--primary)]", frameWidth > 0 ? "rounded-xl" : "")}
                            style={{ width: frameWidth > 0 ? frameWidth : "100%", minHeight: frameWidth > 0 ? 480 : "100%" }}
                        >
                            {showNavbar && (
                                <MockNavbar
                                    pages={state.pages}
